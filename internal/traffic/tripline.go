...
// Vehicles crossing it (entry then exit) yield a speed sample.
...
type SpeedSample struct {
	SegmentID int64
	Time      int64   // Unix nanoseconds, taken at entry crossing
	Speed     float64 // m/s
}
...
const (
	...
	// maxRealisticSpeedMS is the upper bound for plausible vehicle speeds.
	...
	// defaultTripLineWidthMeters is the full perpendicular span used when
	// CreateTripLine is called without an explicit width.
	defaultTripLineWidthMeters = 20.0
)

// LineIntersection computes the parametric intersection of two line segments
// p1→p2 and p3→p4. It returns (t, u, ok) where t is the fraction along
// p1→p2 and u is the fraction along p3→p4. ok is true only when both
// parameters are in [0, 1] (i.e. the segments actually cross).
...
// Bearing returns the initial bearing (degrees, 0–360) from p1 to p2.
...
// id is a unique identifier for this line; segmentID and index tie it to the
// road network. widthMeters is the full perpendicular span (half on each side).
...
// CheckCrossing tests whether the GPS segment p0→p1 (with timestamps t0, t1
// in nanoseconds) crosses this TripLine. Returns a Crossing with an
// interpolated timestamp, or nil if there is no intersection.
func (tl *TripLine) CheckCrossing(p0, p1 Point, t0, t1 int64) *Crossing {
	...
	return &Crossing{
		...
		// VehicleID is filled in by the caller
	}
}
...
func ComputeSpeed(c1, c2 *Crossing) *SpeedSample {
	...
	// Enforce entry (index 1) before exit (index 2).
...
// normalizeBearing clamps a bearing to [0, 360).
